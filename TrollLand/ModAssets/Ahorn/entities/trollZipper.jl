module TrollLandTrollZipMover

using ..Ahorn, Maple

@mapdef Entity "TrollLand/TrollZipMover" TrollZipMover(x::Integer, y::Integer, width::Integer=16, height::Integer=16, theme::String="Normal", trollOffset::Integer=0, trollFixed::Bool=false, switchTroll::Bool=false, flipDirection::Bool=false)

const placements = Ahorn.PlacementDict(
    "Troll Zip Mover ($(uppercasefirst(theme))) (Troll Land)" => Ahorn.EntityPlacement(
        TrollZipMover,
        "rectangle",
        Dict{String, Any}(
            "theme" => theme
...
Ahorn.editingOptions(entity::TrollZipMover) = Dict{String, Any}(
    "theme" => Maple.zip_mover_themes
)

Ahorn.nodeLimits(entity::TrollZipMover) = 1, 1

Ahorn.minimumSize(entity::TrollZipMover) = 16, 16
Ahorn.resizable(entity::TrollZipMover) = true, true

function Ahorn.selection(entity::TrollZipMover)
...
function getTextures(entity::TrollZipMover)
...
function renderZipMover(ctx::Ahorn.Cairo.CairoContext, entity::TrollZipMover)
    x, y = Ahorn.position(entity)
    nx, ny = Int.(entity.data["nodes"][1])
...
function Ahorn.renderAbs(ctx::Ahorn.Cairo.CairoContext, entity::TrollZipMover, room::Maple.Room)
