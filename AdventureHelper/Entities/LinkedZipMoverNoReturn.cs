...
namespace Celeste.Mod.AdventureHelper.Entities
{
    ...
    {
        public LinkedZipMoverNoReturn(Vector2 position, int width, int height, Vector2 target, string colorCode, float speedMultiplier, string spritePath) : base(position, (float)width, (float)height, false)
        {
            spritePath.Trim('/');
            spritePath.Trim('\\');
        ...
        public LinkedZipMoverNoReturn(EntityData data, Vector2 offset) : this(data.Position + offset, data.Width, data.Height, data.Nodes[0] + offset, data.Attr("colorCode", "000000"), data.Float("speedMultiplier", 1f), data.Attr("spritePath", defaultPath))
        {
        }
        ...
        {
            base.Added(scene);
            scene.Add(this.pathRenderer = new LinkedZipMoverNoReturn.ZipMoverPathRenderer(this, spritePath));
        }
            ...
            if (flag)
            {
                ...
                if (flag4)
                {
                    ...
                    if (flag8)
                    {
                        ...
                        {
                            base.SceneAs<Level>().ParticlesFG.Emit(LinkedZipMoverNoReturn.P_Scrape, base.TopLeft + new Vector2(0f, (float)i + (float)num * 2f), (num == 1) ? -0.7853982f : 0.7853982f);
                        }
                    }
                    ...
                    if (flag9)
                    {
                        ...
                        {
                            base.SceneAs<Level>().ParticlesFG.Emit(LinkedZipMoverNoReturn.P_Scrape, base.TopRight + new Vector2(-1f, (float)j + (float)num * 2f), (num == 1) ? -2.3561945f : 2.3561945f);
                        }
                    }
                }
                else
                {
                    ...
                    if (flag10)
                    {
                        ...
                        if (flag14)
                        {
                            ...
                            {
                                base.SceneAs<Level>().ParticlesFG.Emit(LinkedZipMoverNoReturn.P_Scrape, base.TopLeft + new Vector2((float)k + (float)num4 * 2f, -1f), (num4 == 1) ? 2.3561945f : 0.7853982f);
                            }
                        }
                        ...
                        if (flag15)
                        {
                            ...
                            {
                                base.SceneAs<Level>().ParticlesFG.Emit(LinkedZipMoverNoReturn.P_Scrape, base.BottomLeft + new Vector2((float)l + (float)num4 * 2f, 0f), (num4 == 1) ? -2.3561945f : -0.7853982f);
                            }
                        }
                    }
                }
        ...
        private LinkedZipMoverNoReturn.ZipMoverPathRenderer pathRenderer;
        ...
        private static SoundSource sfx = new SoundSource();
        ...
        private class ZipMoverPathRenderer : Entity
        {
            ...
            {
                base.SceneAs<Level>().ParticlesBG.Emit(LinkedZipMoverNoReturn.P_Sparks, this.from + this.sparkAdd + Calc.Random.Range(-Vector2.One, Vector2.One), this.sparkDirFromA);
                base.SceneAs<Level>().ParticlesBG.Emit(LinkedZipMoverNoReturn.P_Sparks, this.from - this.sparkAdd + Calc.Random.Range(-Vector2.One, Vector2.One), this.sparkDirFromB);
                base.SceneAs<Level>().ParticlesBG.Emit(LinkedZipMoverNoReturn.P_Sparks, this.to + this.sparkAdd + Calc.Random.Range(-Vector2.One, Vector2.One), this.sparkDirToA);
                base.SceneAs<Level>().ParticlesBG.Emit(LinkedZipMoverNoReturn.P_Sparks, this.to - this.sparkAdd + Calc.Random.Range(-Vector2.One, Vector2.One), this.sparkDirToB);
            }
            ...
            public LinkedZipMoverNoReturn ZipMover;
